...
仓库内置 GitHub Actions `CI` 工作流，会在 PR 和推送到 `main` 时执行：
...
无论使用 GitHub 页面手动触发，还是本地手动推 tag，推送 tag 后都会由 GitHub Actions 自动创建对应的 draft release。
